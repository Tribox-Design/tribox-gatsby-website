    ...
    data.allMarkdownRemark.edges.forEach((item, index) => {
      ...
      if (item.node.frontmatter.isHighlighted) {
        worksArray.push(
            ...
            <WorkCard key={index}>
            <WorkLink to={item.node.frontmatter.path}><WorkImg 
                alt={item.node.frontmatter.title}
                src={item.node.frontmatter.thumbnail.childImageSharp.fluid.src} 
                fluid={item.node.frontmatter.thumbnail.childImageSharp.fluid}/></WorkLink>
              
              <WorkInfo>
              <WorkLink to={item.node.frontmatter.path}><WorkTitle>{item.node.frontmatter.title}</WorkTitle></WorkLink>
